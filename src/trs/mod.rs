            ...
            egraph[subst[a]].nodes.iter().any(|n1| match n1 {
                Math::Constant(_) => egraph[subst[b]].nodes.iter().any(|n2| match n2 {
                    Math::Constant(_) => egraph[subst[c]].nodes.iter().any(|n3| match n3 {
                        Math::Constant(_) => egraph[subst[x]].nodes.iter().any(|n4| match n4 {
                            Math::Symbol(vx) => egraph[subst[y]].nodes.iter().any(|n5| match n5 {
                                    Math::Symbol(vy) => (vx != vy),
                                    _ => false,
                            }),
                            _ => false,
                        }),
...
pub fn check_impo(egraph: &EGraph, start_id: Id) -> (bool, String) {
    let impossibles = [
        ...
        write_impo!("(!= (/ (+ ?a (- ?x ?y)) ?b) ?c)"; impossible_conditions("x!=y+bc-a", &vec!["?a","?b","?c","?x","?y"])),
...
pub fn prove_faster(
    ...
    if use_iteration_check {
        let (runner_temp, impo_time) = Runner::default()
            ...
            .with_time_limit(Duration::new(params.2, 0))
            .with_expr(&start)
            .run_faster(rules(ruleset_class).iter(), &goals, check_impo);
        runner = runner_temp;
        total_time += impo_time;
    } else {
        runner = Runner::default()
